...
export function modelSupportsToolCalling(model: string): boolean {
  ...
  // OpenRouter's Perplexity/Sonar models are search-answering chat models and
  // reject requests that include tool schemas. Keep normal chat working by
  // omitting OpenBoard's optional canvas tools for that provider.
  return !id.startsWith('perplexity/')
}
...
export function buildSystemPrompt(opts: {
  ...
  if (opts.canvasTools ?? true) {
    lines.push('', CANVAS_TOOLS_PROMPT)
  }
